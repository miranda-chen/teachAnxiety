...
This is a vignette of how to use the teachAnxiety package and its functions to process data collected using the Parson's (1973) Teaching Anxiety Instrument. This vignette can be used as a reference for survey data processing in preparation for subsequent analyses. 
**Add some more text, with literature, about the decisions you made and the overall workflow for processing responses to this survey. Maybe even include the survey instrument itself?**
...
**teachAnxiety** can be installed from CRAN using `install.packages("teachAnxiety")` or from github using `install::github('miranda-chen/teachAnxiety)`.

## Initial steps  


**nope** We will use a variety of packages and sets of packages, in which teachAnxiety functions rely on. These include:**dplyr**, **devtools**, **readr** and **readxl**. Technically, these packages do not need to be loaded with `library()`, however, as they have been included as dependencies within the package itself.
